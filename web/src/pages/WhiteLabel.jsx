    ...
    <div className="space-y-2 animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
        <div>
          <h1 className="text-base md:text-sm font-bold text-white flex items-center gap-2">
            <div className="h-8 w-8 rounded-md bg-yellow-500/10 flex items-center justify-center border border-yellow-500/20">
        ...
        {/* Badge Novo */}
        <span className="inline-flex items-center gap-1.5 text-xs font-bold bg-yellow-500/10 text-yellow-400 px-2 py-1 rounded-md border border-yellow-500/30 animate-pulse">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          ...
          <div key={i} className="glass-effect rounded-md p-2 border border-white/5">
            <stat.icon className={`h-5 w-5 ${stat.cor} mb-2`} />
            <p className={`text-sm font-black ${stat.cor}`}>{stat.value}</p>
      ...
      <div className="flex gap-2 border-b border-white/5 pb-0.5">
        {TABS.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-2 py-1 text-sm font-bold rounded-t-lg border-b-2 transition-all ${
              tab === t.id
      ...
      {tab === 'identidade' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {/* Identidade Visual */}
          <div className="glass-effect rounded-md p-5 border border-white/5 space-y-2">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-dark-900 border border-white/10 rounded-md py-1.5 px-4 text-white text-sm focus:border-brand/50 focus:outline-none transition placeholder-zinc-600"
            ...
            <div>
              ...
              <label className="cursor-pointer block">
                <div className={`border-2 border-dashed rounded-md p-5 text-center transition ${logo ? 'border-brand/40 bg-brand/5' : 'border-white/10 hover:border-brand/30 hover:bg-brand/5'}`}>
                  {logo ? (
                    <img src={logo} alt="Logo" className="h-14 mx-auto object-contain rounded-md" />
                  ) : (
                    <>
                      <Upload className="h-5 w-5 mx-auto text-zinc-600 mb-2" />
                      <p className="text-xs text-zinc-400">Clique para enviar sua logo</p>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {TEMAS.map(tema => (
                  ...
                  >
                    <div className={`h-9 bg-gradient-to-br ${tema.preview} flex items-end p-1.5`}>
          ...
          <div className="space-y-2">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-dark-900 border border-white/10 rounded-md py-1.5 pl-10 pr-4 text-white text-sm focus:border-green-500/50 focus:outline-none transition placeholder-zinc-600"
            ...
            <div className="glass-effect rounded-md p-5 border border-white/5 space-y-2">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="bg-dark-900 border border-white/10 rounded-md px-2 py-1.5 text-zinc-500 text-sm whitespace-nowrap">
                    loja.mxxcontrol.com/
                  </div>
                  <input
                    type="text"
                    ...
                    className="flex-1 bg-dark-900 border border-white/10 rounded-md py-1 px-2 text-white text-sm focus:border-brand/50 focus:outline-none transition placeholder-zinc-600"
              ...
              {/* Prévia do Link */}
              <div className="bg-dark-900/50 rounded-md p-2 border border-white/5 flex items-center justify-between gap-2">
            ...
            <button
              ...
              disabled={saving}
              className="w-full flex items-center justify-center gap-2 py-1.5 rounded-md font-bold text-sm shadow-lg transition transform active:scale-95 bg-brand hover:bg-brand/90 text-white disabled:opacity-60"
            >
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {planos.map(plano => {
              ...
              return (
                <div key={plano.id} className={`glass-effect rounded-md border-2 p-5 space-y-2 relative transition-all hover:-translate-y-1 ${cor.border}`}>
                  {plano.badge && (
                    <span className={`absolute -top-2 left-1/2 -translate-x-1/2 text-[10px] font-black tracking-wider px-3 py-1 rounded-full ${cor.btn} text-white shadow-lg ${cor.glow} shadow-md`}>
                      {plano.badge}
                    </span>
                  )}

                  <div className="flex justify-between items-start">
                    <div className={`h-8 w-8 rounded-md ${cor.bg} flex items-center justify-center border ${cor.border}`}>
                      <Package className={`h-5 w-5 ${cor.text}`} />
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => handleEditPlan(plano)} className="h-5 w-5 rounded-md bg-dark-700/50 hover:bg-dark-600 text-zinc-400 hover:text-white flex items-center justify-center transition border border-dark-600"><Edit2 className="h-3.5 w-3.5" /></button>
                      <button onClick={() => handleDeletePlan(plano.id)} className="h-5 w-5 rounded-md bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white flex items-center justify-center transition border border-red-500/20 hover:border-red-500"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div>
                  </div>
                  ...
                  <div>
                    <span className={`text-sm font-black ${cor.text}`}>{plano.preco}</span>
          ...
          <div onClick={handleCreatePlan} className="glass-effect rounded-md border border-dashed border-white/10 p-5 text-center hover:border-brand/30 hover:bg-brand/5 transition cursor-pointer group flex flex-col items-center justify-center">
            <div className="h-8 w-8 rounded-full bg-dark-800 border border-dark-600 flex items-center justify-center group-hover:bg-brand group-hover:border-brand mb-2 transition shadow-lg">
              <Plus className="h-5 w-5 text-zinc-500 group-hover:text-white transition" />
            </div>
          ...
          {showPlanModal && (
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-2 z-50 animate-in fade-in duration-200">
              <div className="glass-effect relative w-full max-w-lg shadow-2xl rounded-md border border-dark-700 bg-dark-900/95 p-1 flex flex-col max-h-[90vh]">
                <div className="px-2 py-1 border-b border-dark-700 bg-dark-800/50 flex justify-between items-center rounded-t-2xl shrink-0">
                ...
                <form onSubmit={handleSavePlan} className="p-5 flex-1 overflow-y-auto custom-scrollbar space-y-2">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div>
                      <label className="text-xs font-bold text-zinc-500 uppercase block mb-1">Nome do Plano *</label>
                      <input type="text" required value={planForm.nome} onChange={e => setPlanForm({...planForm, nome: e.target.value})} className="w-full bg-dark-900 border border-dark-600 rounded-md px-2 py-1 text-white text-sm focus:border-brand/50 focus:outline-none" placeholder="Ex: Mensal Básico" />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-zinc-500 uppercase block mb-1">Preço *</label>
                      <input type="text" required value={planForm.preco} onChange={e => setPlanForm({...planForm, preco: e.target.value})} className="w-full bg-dark-900 border border-dark-600 rounded-md px-2 py-1 text-white text-sm font-bold focus:border-brand/50 focus:outline-none" placeholder="R$ 35,00" />
                    </div>
                  </div>

                  <div>
                    <label className="text-xs font-bold text-zinc-500 uppercase block mb-1">Subtítulo (Opcional)</label>
                    <input type="text" value={planForm.descricao} onChange={e => setPlanForm({...planForm, descricao: e.target.value})} className="w-full bg-dark-900 border border-dark-600 rounded-md px-2 py-1 text-white text-sm focus:border-brand/50 focus:outline-none" placeholder="Ex: Acesso completo por 30 dias" />
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div>
                      <label className="text-xs font-bold text-zinc-500 uppercase block mb-1">Badge de Destaque</label>
                      <input type="text" value={planForm.badge} onChange={e => setPlanForm({...planForm, badge: e.target.value})} className="w-full bg-dark-900 border border-dark-600 rounded-md px-2 py-1 text-white text-sm focus:border-brand/50 focus:outline-none" placeholder="Ex: Mais Vendido" />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-zinc-500 uppercase block mb-1">Cor Temática</label>
                      <select value={planForm.cor} onChange={e => setPlanForm({...planForm, cor: e.target.value})} className="w-full bg-dark-900 border border-dark-600 rounded-md px-2 py-1 text-white text-sm focus:border-brand/50 focus:outline-none">
                  ...
                  <div>
                    ...
                    <textarea required value={planForm.features} onChange={e => setPlanForm({...planForm, features: e.target.value})} className="w-full bg-dark-900 border border-dark-600 rounded-md px-2 py-1 text-white text-sm h-32 focus:border-brand/50 focus:outline-none custom-scrollbar" placeholder="Sinal em 4K&#10;Mais de 100.000 títulos&#10;Suporte VIP 24/7"></textarea>
                  </div>
                  
                  <div className="pt-4 border-t border-dark-700 flex justify-end gap-2 sticky bottom-0 bg-dark-900/95 py-1.5 backdrop-blur-sm">
                    <button type="button" onClick={() => setShowPlanModal(false)} className="px-4 py-1.5 bg-dark-700 text-zinc-300 font-bold rounded-md border border-dark-600 text-sm">Cancelar</button>
                    <button type="submit" className="px-4 py-1.5 bg-brand text-white font-bold rounded-md shadow border border-brand/50 text-sm">Salvar Plano</button>
                  </div>
                </form>
              </div>
            </div>
      ...
      {tab === 'preview' && (
        <div className="space-y-2">
          ...
          <div
            className="rounded-md overflow-hidden border border-white/10 shadow-2xl"
          ...
          >
            ...
            >
              {logo ? (
                ...
                <div className="flex items-center gap-2">
                  <div className="h-5 w-5 rounded-md flex items-center justify-center" style={{ background: temaSel.primary }}>
            ...
            <div className="py-12 px-6 text-center">
              <h1 className="text-base md:text-sm font-black text-white mb-2">{nomeLoja}</h1>
              ...
              <button
                className="px-6 py-1.5 rounded-md font-bold text-white text-sm shadow-lg"
            ...
            {/* Planos Preview */}
            <div className="px-6 pb-12 grid grid-cols-1 md:grid-cols-3 gap-2">
              {planos.map(plano => (
                <div
                  key={plano.id}
                  className="rounded-md p-2 border text-center transition hover:-translate-y-1 shadow-lg cursor-pointer"
                ...
                >
                  ...
                  <p className="font-black text-white text-base mt-1" style={{ color: temaSel.primary }}>{plano.preco}</p>
                  
                  <ul className="space-y-1.5 mt-2 mb-2 text-left">
                  ...
                  <button
                    className="w-full mt-2 py-1.5 rounded-md text-xs font-bold text-white shadow"
                    style={{ background: temaSel.primary }}
                  >
                    Contratar
                  </button>
