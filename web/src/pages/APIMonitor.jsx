...
const StatCard = ({ label, value, color, Icon }) => (
  <div style={{ background:'rgba(17,17,17,0.7)', backdropFilter:'blur(14px)', border:'1px solid rgba(252, 95, 22,0.1)', borderRadius:14, padding:'18px 22px', display:'flex', alignItems:'center', gap:14, boxShadow:'0 4px 20px rgba(0,0,0,0.3)' }}>
    ...
    <div>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:24, flexWrap:'wrap', gap:12 }}>
        <div>
          <h1 style={{ fontSize:26, fontWeight:900, color:'#fff', display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
            <Activity size={26} color='#FC5F16'/> Monitor de APIs
          </h1>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer' }}>
            <div onClick={() => setAutoRefresh(v=>!v)} style={{ width:38, height:20, borderRadius:999, background: autoRefresh?'rgba(252, 95, 22,0.3)':'rgba(255,255,255,0.08)', border:`1px solid ${autoRefresh?'rgba(252, 95, 22,0.5)':'rgba(255,255,255,0.1)'}`, position:'relative', transition:'all .2s', cursor:'pointer', flexShrink:0 }}>
              <div style={{ width:14, height:14, borderRadius:'50%', background: autoRefresh?'#FC5F16':'#52525b', position:'absolute', top:2, left: autoRefresh?20:2, transition:'all .2s' }}/>
            </div>
            <span style={{ fontSize:12, color:'#71717a' }}>Auto (30s)</span>
          </label>
          <button onClick={loadData} style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'9px 16px', background:'rgba(252, 95, 22,0.1)', border:'1px solid rgba(252, 95, 22,0.2)', borderRadius:10, color:'#FC5F16', fontSize:13, fontWeight:700, cursor:'pointer' }}>
      ...
      {data?.summary && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:12, marginBottom:24 }}>
          <StatCard label='Total de APIs'   value={data.summary.total}         color='#FC5F16' Icon={Activity}     />
      ...
      {loading && !data ? (
        ...
        <div style={{ background:'rgba(17,17,17,0.7)', backdropFilter:'blur(14px)', border:'1px solid rgba(252, 95, 22,0.1)', borderRadius:16, overflow:'hidden', boxShadow:'0 8px 32px rgba(0,0,0,0.3)' }}>
