...
const IptvTreeViewer = () => {
  ...
  const handleSaveConfig = async (e) => {
    ...
    try {
      if (!editProvider.id) {
         // Se não tem ID, é um slot novo ou não inicializado
  ...
  return (
    <div className="p-6 bg-[#050505] min-h-screen text-white">
      {/* 🎰 SIX SLOTS MASTER (FORCE DISPLAY) */}
      <div className="flex gap-4 mb-8 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-orange-500/20">
        {[1,2,3,4,5,6].map(i => {
          ...
          return (
            <button 
              ...
              className={`flex-shrink-0 w-44 p-5 rounded-3xl border-2 transition-all duration-300 flex flex-col items-center gap-2 group ${
                active 
                ? 'bg-orange-500/10 border-orange-500 text-orange-500 shadow-[0_0_25px_rgba(255,165,0,0.15)] scale-105' 
                : 'bg-[#111] border-white/5 text-zinc-500 hover:border-white/10'
              }`}
            >
              <div className={`p-3 rounded-2xl ${active ? 'bg-orange-500 text-black' : 'bg-[#222] group-hover:bg-[#333]'}`}>
                 <Monitor size={24} />
              </div>
        ...
        <button 
          onClick={() => { setEditProvider(currentProvider); setShowConfig(true); }}
          className="flex-shrink-0 w-16 bg-[#181818] border border-white/5 rounded-3xl flex items-center justify-center hover:bg-orange-500 hover:text-black transition-all"
        >
          <Settings size={22} />
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-black text-white flex items-center gap-4">
            <span className="p-3 bg-orange-500 text-black rounded-2xl shadow-[0_0_20px_rgba(255,165,0,0.3)]">
        ...
        <button 
          onClick={loadRoot}
          className="px-6 py-3 bg-[#111] border border-white/10 rounded-2xl text-sm font-bold flex items-center gap-2 hover:bg-white/5 transition-colors"
        >
              ...
              <div className="space-y-2 relative z-10">
                {treeData.length === 0 && !loading && (
                   <div className="py-20 text-center">
                      ...
                      <p className="text-zinc-500 max-w-xs mx-auto">Configure os dados do servidor clicando na engrenagem no topo.</p>
                   </div>
                )}
                  ...
                  treeData.map(node => (
                    <div key={node.id} className="border-b border-white/5 last:border-0 pb-1">
                      <div 
                        ...
                        className={`flex items-center p-4 cursor-pointer hover:bg-white/5 rounded-2xl transition-all group ${expandedNodes.has(node.id) ? 'bg-white/5' : ''}`}
                      >
                        <div className={`p-2 rounded-lg mr-4 ${expandedNodes.has(node.id) ? 'bg-orange-500 text-black' : 'bg-[#222] text-zinc-500'}`}>
                        ...
                        <div className="ml-12 py-4 mr-4 grid grid-cols-1 md:grid-cols-2 gap-3 border-l-2 border-orange-500/20 pl-6">
                          {node.children.map(child => (
                            <div key={child.id} className="flex items-center p-3 bg-white/5 hover:bg-white/10 rounded-2xl gap-4 border border-transparent hover:border-orange-500/20 transition-all group/item">
                              <div className="w-10 h-10 rounded-xl bg-black/40 overflow-hidden flex-shrink-0 border border-white/10 group-hover/item:border-orange-500/50 transition-colors">
                                 {child.stream_icon ? <img src={child.stream_icon} className="w-full h-full object-cover" alt="" /> : <Film size={20} className="m-2.5 text-zinc-600" />}
                              </div>
                              <span className="flex-1 text-zinc-300 text-sm font-bold truncate group-hover/item:text-white">{child.name}</span>
                              <button 
                                onClick={(e) => { e.stopPropagation(); addToCuration(child); }} 
                                className="px-4 py-2 bg-orange-500 text-black text-[10px] font-black rounded-xl uppercase hover:scale-105 active:scale-95 transition-all shadow-lg shadow-orange-500/10"
                              >
                                🎬 + Banner
                              </button>
                            </div>
      ...
      {/* MODAL CONFIGURAÇÃO SLOT PRO MAX */}
      {showConfig && editProvider && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl flex items-center justify-center z-50 p-6">
          <form onSubmit={handleSaveConfig} className="bg-[#111] p-10 rounded-[2.5rem] border-2 border-orange-500/30 w-full max-w-lg space-y-6 shadow-[0_0_100px_rgba(255,165,0,0.1)] relative">
            <div className="absolute -top-10 left-10 p-5 bg-orange-500 text-black rounded-3xl shadow-2xl">
               <Settings size={32} />
            </div>
            
            <div className="pt-4">
               <h2 className="text-white font-black text-3xl uppercase tracking-tighter">Configurar Slot {activeSlot}</h2>
               <p className="text-zinc-500 text-sm mt-1">Insira os dados do servidor Xtream para este agente.</p>
            </div>
            
            <div className="space-y-4">
               <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase text-zinc-500 ml-1">Nome do Servidor</label>
               ...
               <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase text-zinc-500 ml-1">URL (DNS)</label>
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                     <label className="text-[10px] font-black uppercase text-zinc-500 ml-1">Usuário</label>
            ...
            <div className="flex gap-4 pt-6">
              <button type="submit" className="flex-1 bg-orange-500 text-black font-black p-5 rounded-2xl hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-orange-500/20 uppercase tracking-widest text-sm">SALVAR CONFIGURAÇÃO</button>
