...
const IptvTreeViewer = () => {
  ...
  return (
    <div className="p-5 bg-[#050505] min-h-screen text-white">
      <div className="flex gap-4 mb-6 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-orange-500/20">
        {[1, 2, 3, 4, 5, 6].map(i => {
          ...
          return (
            <button key={i} onClick={() => setActiveSlot(i)} className={`flex-shrink-0 w-44 p-5 rounded-3xl border-2 transition-all duration-300 flex flex-col items-center gap-2 group ${active ? 'bg-orange-500/10 border-orange-500 text-orange-500 shadow-[0_0_25px_rgba(252, 95, 22,0.15)] scale-105' : 'bg-[#111] border-white/5 text-zinc-500 hover:border-white/10'}`}>
              <div className={`p-3 rounded-xl ${active ? 'bg-orange-500 text-black' : 'bg-[#222] group-hover:bg-[#333]'}`}><Monitor size={24} /></div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-4">
            <span className="p-3 bg-orange-500 text-black rounded-xl shadow-[0_0_20px_rgba(252, 95, 22,0.3)]"><Star size={24} fill="currentColor" /></span>
        ...
        <button onClick={loadRoot} className="px-6 py-3 bg-[#111] border border-white/10 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-white/5 transition-colors">
                ...
                {loading ? (
                  ...
                  treeData.map(node => (
                    <div key={node.id} className="border-b border-white/5 last:border-0 pb-1">
                      <div onClick={() => toggleNode(node)} className={`flex items-center p-4 cursor-pointer hover:bg-white/5 rounded-xl transition-all group ${expandedNodes.has(node.id) ? 'bg-white/5' : ''}`}>
                      ...
                      {expandedNodes.has(node.id) && node.children && (
                        <div className="ml-12 py-4 mr-4 grid grid-cols-1 md:grid-cols-2 gap-3 border-l-2 border-orange-500/20 pl-6">
                          {node.children.map(child => (
                            <div key={child.id} className="flex items-center p-3 bg-white/5 hover:bg-white/10 rounded-xl gap-4 border border-transparent hover:border-orange-500/20 transition-all group/item">
                              <div className="w-10 h-10 rounded-lg bg-black/40 overflow-hidden flex-shrink-0 border border-white/10">{child.stream_icon ? <img src={child.stream_icon} className="w-full h-full object-cover" alt="" /> : <Film size={20} className="m-2.5 text-zinc-600" />}</div>
                              <span className="flex-1 text-zinc-300 text-sm font-bold truncate group-hover/item:text-white">{child.name}</span>
                              <button onClick={(e) => { e.stopPropagation(); addToCuration(child); }} className="px-4 py-2 bg-orange-500 text-black text-[10px] font-black rounded-lg uppercase hover:scale-105 active:scale-95 transition-all shadow-lg shadow-orange-500/10">🎬 + Banner</button>
                            </div>
                          ))}
                        </div>
                      )}
      ...
      {showConfig && editProvider && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl flex items-center justify-center z-50 p-5">
          <form onSubmit={handleSaveConfig} className="bg-[#111] p-10 rounded-[2.5rem] border-2 border-orange-500/30 w-full max-w-lg space-y-6 shadow-[0_0_100px_rgba(252, 95, 22,0.1)] relative">
            <h2 className="text-white font-black text-2xl uppercase tracking-tighter">Configurar Slot {activeSlot}</h2>
            <div className="space-y-4">
              <input className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white outline-none focus:border-orange-500 transition-colors font-bold" value={editProvider.name || ''} onChange={e => setEditProvider({ ...editProvider, name: e.target.value })} placeholder="Ex: Servidor VIP" />
              <input className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white outline-none focus:border-orange-500 transition-colors font-mono text-sm" value={editProvider.url || ''} onChange={e => setEditProvider({ ...editProvider, url: e.target.value })} placeholder="http://dominio.com:8080" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <input className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white outline-none focus:border-orange-500 transition-colors font-bold" value={editProvider.username || ''} onChange={e => setEditProvider({ ...editProvider, username: e.target.value })} placeholder="Username" />
                <input className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white outline-none focus:border-orange-500 transition-colors font-bold" value={editProvider.password || ''} onChange={e => setEditProvider({ ...editProvider, password: e.target.value })} placeholder="Password" type="password" />
              </div>
            </div>
            <div className="flex gap-4 pt-6">
              <button type="submit" className="flex-1 bg-orange-500 text-black font-black p-5 rounded-xl hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-widest text-sm">SALVAR CONFIGURAÇÃO</button>
              <button type="button" onClick={() => setShowConfig(false)} className="px-6 bg-zinc-800 text-white font-bold p-5 rounded-xl hover:bg-zinc-700 transition-all uppercase tracking-widest text-xs">CANCELAR</button>
            </div>
          </form>
        </div>
      )}
