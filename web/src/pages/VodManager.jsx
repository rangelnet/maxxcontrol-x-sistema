...
const VodManager = () => {
  ...
  const fetchConfig = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/ui/vod');
  ...
  const addCategory = () => {
    setConfig({
      ...config,
      featuredCategories: [...config.featuredCategories, { id: Date.now().toString(), name: 'Nova Categoria', type: 'iptv_category', categoryId: '', active: true }]
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-white">Layout de Filmes (VOD)</h2>
          <p className="text-sm text-zinc-400">Gerencie as categorias em destaque na tela de filmes.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 space-y-6">
          ...
          <h3 className="text-lg font-bold text-white flex items-center gap-2">🎬 Categorias em Destaque</h3>
        ...
        <div className="xl:col-span-1 space-y-6">
          <PreviewContainer title="Filmes (VOD)">
            <VodSeriesPreview featuredCategories={config.featuredCategories} type="vod" />
          </PreviewContainer>
        </div>
      </div>
    </div>
