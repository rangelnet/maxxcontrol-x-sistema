  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-2">
        <h1 className="text-sm font-bold">APIs</h1>
        {activeTab === 'config' && (
      ...
      <div className="flex gap-1 mb-2 bg-gray-900 p-1 rounded-md w-fit">
        {canConfig && (
          <button onClick={() => setActiveTab('config')} className={`px-5 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === 'config' ? 'bg-[#FF4D33] text-white' : 'text-gray-400 hover:text-white'}`}>Configuracao</button>
        )}
        {canMonitor && (
          <button onClick={() => setActiveTab('monitor')} className={`px-5 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'monitor' ? 'bg-[#FF4D33] text-white' : 'text-gray-400 hover:text-white'}`}><Activity size={16} />Monitor</button>
        )}
      </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="text-sm font-bold">{a.nome}</h3>
                  ...
                  <div className="flex gap-2 text-xs text-gray-500"><span>Metodo: {a.metodo}</span><span>Timeout: {a.timeout}ms</span></div>
                </div>
            ...
            <>
              {monitorData?.summary && (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-2 mb-2">
                  <div className="card"><div className="flex items-center justify-between"><div><p className="text-gray-400 text-sm mb-1">Total de APIs</p><p className="text-sm font-bold">{monitorData.summary.total}</p></div><Activity className="text-blue-500" size={40} /></div></div>
                  <div className="card"><div className="flex items-center justify-between"><div><p className="text-gray-400 text-sm mb-1">Online</p><p className="text-sm font-bold text-green-500">{monitorData.summary.online}</p></div><CheckCircle className="text-green-500" size={40} /></div></div>
                  <div className="card"><div className="flex items-center justify-between"><div><p className="text-gray-400 text-sm mb-1">Offline</p><p className="text-sm font-bold text-red-500">{monitorData.summary.offline}</p></div><XCircle className="text-red-500" size={40} /></div></div>
                  <div className="card"><div className="flex items-center justify-between"><div><p className="text-gray-400 text-sm mb-1">Latencia Media</p><p className="text-sm font-bold">{monitorData.summary.avg_latency}ms</p></div><Clock className="text-primary" size={40} /></div></div>
                </div>
              )}
              {monitorData?.summary?.critical_offline > 0 && (
                <div className="card bg-red-500/10 border-red-500 mb-2">
                  <div className="flex items-center gap-2"><AlertTriangle className="text-red-500" size={24} /><div><p className="font-bold text-red-500">Atencao!</p><p className="text-sm text-gray-300">{monitorData.summary.critical_offline} API(s) critica(s) estao offline</p></div></div>
                </div>
              )}
              <div className="card">
                <h2 className="text-sm font-bold mb-2">Status das APIs</h2>
                <div className="space-y-2">
                    ...
                    <div key={i} className="flex items-center justify-between p-2 bg-gray-900 rounded">
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-400 font-mono">{a.url}</p>
                        </div>
                      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-2 z-50">
          <div className="card max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-sm font-bold">{editingAPI ? 'Editar API' : 'Nova API'}</h2>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-white"><X size={24} /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-2">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="col-span-1 sm:col-span-2"><label className="block text-sm font-medium mb-2">Nome da API *</label><input type="text" value={formData.nome} onChange={(e) => setFormData({...formData, nome: e.target.value})} className="w-full px-4 py-1.5 bg-gray-800 border border-gray-700 rounded" placeholder="Ex: Auth API" required /></div>
                <div className="col-span-1 sm:col-span-2"><label className="block text-sm font-medium mb-2">Descricao</label><textarea value={formData.descricao} onChange={(e) => setFormData({...formData, descricao: e.target.value})} className="w-full px-4 py-1.5 bg-gray-800 border border-gray-700 rounded" rows="2" /></div>
                <div className="col-span-1 sm:col-span-2"><label className="block text-sm font-medium mb-2">URL *</label><input type="url" value={formData.url} onChange={(e) => setFormData({...formData, url: e.target.value})} className="w-full px-4 py-1.5 bg-gray-800 border border-gray-700 rounded" placeholder="https://api.exemplo.com/" required /></div>
                <div><label className="block text-sm font-medium mb-2">Categoria</label><input type="text" value={formData.categoria} onChange={(e) => setFormData({...formData, categoria: e.target.value})} className="w-full px-4 py-1.5 bg-gray-800 border border-gray-700 rounded" list="cats" /><datalist id="cats">{categories.map(c => <option key={c} value={c} />)}</datalist></div>
                <div><label className="block text-sm font-medium mb-2">Metodo HTTP</label><select value={formData.metodo} onChange={(e) => setFormData({...formData, metodo: e.target.value})} className="w-full px-4 py-1.5 bg-gray-800 border border-gray-700 rounded"><option>GET</option><option>POST</option><option>PUT</option><option>DELETE</option><option>PATCH</option></select></div>
                <div><label className="block text-sm font-medium mb-2">Timeout (ms)</label><input type="number" value={formData.timeout} onChange={(e) => setFormData({...formData, timeout: parseInt(e.target.value)})} className="w-full px-4 py-1.5 bg-gray-800 border border-gray-700 rounded" min="1000" max="30000" /></div>
                <div className="col-span-1 sm:col-span-2"><label className="block text-sm font-medium mb-2">Headers (JSON)</label><textarea value={formData.headers} onChange={(e) => setFormData({...formData, headers: e.target.value})} className="w-full px-4 py-1.5 bg-gray-800 border border-gray-700 rounded font-mono text-sm" rows="3" placeholder='{"Authorization": "Bearer token"}' /></div>
