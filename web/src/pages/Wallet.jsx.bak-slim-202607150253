  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        {isMasterOrUnlimited && (
          <div className="flex bg-dark-900 border border-dark-700 rounded-lg p-1 overflow-x-auto">
            <button onClick={() => setActiveTab('extrato')} className={`flex items-center gap-2 px-4 py-2 font-bold text-sm rounded-md transition-all shrink-0 ${activeTab === 'extrato' ? 'bg-dark-700 text-white shadow' : 'text-zinc-500 hover:text-zinc-300'}`}>
                <Clock className="w-4 h-4" /> Extrato
            </button>
            <button onClick={() => setActiveTab('revendas')} className={`flex items-center gap-2 px-4 py-2 font-bold text-sm rounded-md transition-all shrink-0 ${activeTab === 'revendas' ? 'bg-brand-500/20 text-brand-500 border border-brand-500/30' : 'text-zinc-500 hover:text-brand-500/50'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-5 relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="mt-4 pt-4 border-t border-dark-700/50 z-10 flex items-center gap-2">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-dark-900/30 text-zinc-400 text-xs uppercase tracking-wider">
                    ...
                    <th className="p-4 font-medium text-right">Créditos</th>
                  </tr>
                </thead>
                  ...
                  {(transactions || []).map((tx) => {
                    ...
                    return (
                      <tr key={tx.id} className="hover:bg-dark-700/30 transition-colors">
                        ...
                        <td className="p-4 text-sm text-zinc-300 max-w-[200px] truncate">
                          {isSentToOther 
                        ...
                        <td className={`p-4 text-right font-bold ${info.color}`}>
                          {displayCredits}
                        </td>
                      </tr>
                    )
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-lg overflow-hidden animate-in fade-in">
          <div className="p-4 border-b border-dark-700 bg-dark-900/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar revenda..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="bg-dark-900 border border-dark-600 text-white text-sm rounded-lg pl-9 pr-4 py-2 focus:outline-none focus:border-brand-500 w-full md:w-64" />
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-dark-900/30 text-zinc-400 text-xs uppercase tracking-wider">
                  ...
                  <th className="p-4 font-medium text-center">Saldo de Créditos</th>
                ...
                {resellers.filter(r => r.nome?.toLowerCase().includes(searchTerm.toLowerCase()) || r.email?.toLowerCase().includes(searchTerm.toLowerCase())).map((rev) => (
                  <tr key={rev.id} className="hover:bg-dark-700/30 transition-colors">
                    ...
                    <td className="p-4 text-center">
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-500 font-bold">
                    ...
                    <td className="p-4 text-right">
                      <button 
                        ...
                        className={`h-8 px-3 rounded-lg text-[10px] font-black uppercase flex items-center gap-1.5 transition-all ml-auto ${rev.status === 'ativo' ? 'bg-brand-500/10 text-brand-500 hover:bg-brand-500 hover:text-white border border-brand-500/20 active:scale-95' : 'bg-dark-700 text-zinc-600 cursor-not-allowed border border-dark-600'}`}>
          ...
          <div className="bg-dark-900 border border-dark-700 rounded-xl p-5 w-full max-w-sm relative overflow-hidden">
            {transferSuccessMessage ? (
              <div className="text-center py-6 animate-in zoom-in-95 duration-300">
                ...
                <div className="w-20 h-20 bg-emerald-500/20 text-emerald-400 rounded-full flex items-center justify-center mx-auto mb-5 shadow-[0_0_30px_rgba(16,185,129,0.3)]">
                  <CheckCircle size={40} />
                </div>
              ...
              <>
                ...
                <div className="mb-5">
                  <div className="text-sm text-zinc-400 mb-1">Revendedor Destino:</div>
                  <div className="font-bold text-white bg-dark-800 p-3 rounded-lg border border-dark-700">{transferData.nome}</div>
                </div>

                {tfaMode ? (
                   <div>
                      <div className="bg-yellow-500/10 border border-yellow-500/20 text-yellow-500 text-xs p-3 rounded-lg mb-4">
                      ...
                      <input type="text" value={tfaCode} onChange={e => setTfaCode(e.target.value)} className="w-full bg-dark-800 border border-dark-600 text-white rounded-lg px-4 py-3 text-center tracking-widest font-mono text-xl focus:outline-none focus:border-brand-500 mb-4" placeholder="000000" />
                   </div>
                ) : (
                   <div>
                      <label className="block text-xs font-bold text-zinc-400 mb-1.5 uppercase">Quantidade de Créditos</label>
                      <input type="number" min="1" value={transferData.quantidade} onChange={e => setTransferData({...transferData, quantidade: parseInt(e.target.value) || 0})} className="w-full bg-dark-800 border border-dark-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:border-brand-500 mb-4" />
                   </div>
                )}
