...
const ThemeCard = ({ theme, selected, onSelect, tick }) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        onClick={handleClick}
        className={`cursor-pointer group relative rounded-md overflow-hidden aspect-[4/5] border-2 transition-all duration-300 active:scale-[0.97]
          ${isSelected
      ...
      >
        ...
        <div className="absolute inset-0 bg-dark-900 overflow-hidden">
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-base opacity-20">🖼️</span>
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-dark-900/95 border-t border-dark-700 py-1.5 px-2 z-20 backdrop-blur-md">
      ...
      {theme.isGroup && showVariants && (
        <div className="bg-dark-800 border border-dark-700 rounded-md p-2 grid grid-cols-3 gap-2 shadow-inner animate-in slide-in-from-top-2 duration-200">
          {theme.variants.map(v => (
            <button
              key={v.id}
              onClick={() => { onSelect(v.id); setShowVariants(false) }}
              className={`flex flex-col items-center gap-1.5 p-2 rounded-md transition-all hover:bg-dark-700
...
const BannerGenerator = () => {
  ...
  return (
    <div className="space-y-2 pb-20 animate-fade-in">
      {/* ── HEADER & TABS (Marketing Hub Style) ────────────────── */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 bg-dark-800/50 p-5 rounded-[2rem] border border-white/5 shadow-2xl">
        <div>
          <h1 className="text-base md:text-sm font-black text-white tracking-tight flex items-center gap-2">
        ...
        <div className="flex bg-black/40 p-1.5 rounded-md border border-white/10 self-start md:self-center">
          {canGen && (
            <button 
              onClick={() => setActiveTab('generator')}
              className={`px-6 py-1.5 rounded-md text-xs font-black transition-all flex items-center gap-2 uppercase tracking-widest ${activeTab === 'generator' ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20' : 'text-zinc-500 hover:text-white'}`}
            >
          ...
          {canTheme && (
            <button 
              ...
              className={`px-6 py-1.5 rounded-md text-xs font-black transition-all flex items-center gap-2 uppercase tracking-widest ${activeTab === 'themes' ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20' : 'text-zinc-500 hover:text-white'}`}
            >
            ...
            <div className="bg-dark-800/80 backdrop-blur-md border border-brand-500/20 rounded-[2rem] p-5 shadow-xl">
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-xs font-black text-white flex items-center gap-2 uppercase tracking-widest">
                  <span className="text-base">🎬</span> Sua Seleção (Criação Direta)
                </h2>
              ...
              <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-brand-500/20">
                {curationItems.map((item) => (
                  ...
                  >
                    <div className="aspect-[2/3] rounded-md overflow-hidden border-2 border-white/5 group-hover:border-brand-500/50 shadow-lg">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
        {CATEGORIES.map(cat => (
          <button
            ...
            className={`flex items-center gap-2 px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap transition-all border shrink-0
      ...
      {activeCategory === 'futebol' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div
            ...
            className={`group bg-dark-800 border-2 rounded-md p-5 cursor-pointer transition-all duration-300 hover:-translate-y-1 relative overflow-hidden text-center
              ${mode === 'manual'
          ...
          >
            ...
            <div className={`h-8 w-8 rounded-md mx-auto flex items-center justify-center mb-2 border transition-colors
            ...
            <h3 className="text-sm font-black text-white mb-1">Modo Manual</h3>
            <p className="text-zinc-400 text-xs leading-relaxed">
              Escolha campeonato, jogo e personalize nos detalhes.
            </p>
            {mode === 'manual' && (
              <div className="absolute top-2 right-3 w-5 h-5 bg-brand-500 rounded-full flex items-center justify-center">
          ...
          >
            ...
            <h3 className="text-sm font-black text-white mb-1">Modo Automático</h3>
            <p className="text-zinc-400 text-xs leading-relaxed">
              Selecione temas e gere artes de todos os jogos do dia.
            </p>
            {mode === 'auto' && (
              <div className="absolute top-2 right-3 w-5 h-5 bg-green-500 rounded-full flex items-center justify-center">
                <span className="text-white text-[10px]">✓</span>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {mode === 'auto' && (
            <button
              ...
              className="flex items-center gap-2 bg-green-600 hover:bg-green-500 disabled:opacity-50 text-white font-bold py-1.5 px-5 rounded-md transition shadow-lg text-sm"
            >
              {generatingBatch ? (
          ...
          {activeCategory === 'admin' ? (
            ...
            mode === 'auto' ? (
              ...
              <div
                ...
                className={`cursor-pointer relative rounded-md overflow-hidden aspect-[4/5] border-2 transition-all duration-300 active:scale-[0.97]
      ...
      {/* ── RESULTADO LOTE ──────────────────────────────────────── */}
      {batchResult && (
        <div className="bg-gradient-to-r from-green-900/40 to-dark-800 border border-green-500/30 rounded-md p-5 flex flex-col md:flex-row items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 bg-green-500/20 rounded-full flex items-center justify-center text-base border border-green-500/30">
              ✅
            </div>
            <div>
              <h3 className="font-bold text-white text-base">{batchResult.count} artes geradas!</h3>
          ...
          <div className="flex gap-2">
            <button className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white font-bold px-5 py-1.5 rounded-md transition shadow-lg text-sm">
              📥 Baixar ZIP
            </button>
            <button
              onClick={() => setBatchResult(null)}
              className="flex items-center gap-2 bg-dark-700 hover:bg-dark-600 text-zinc-300 font-bold px-2 py-1 rounded-md transition text-sm"
            >
              ✕ Fechar
            </button>
          </div>
      ...
      {(activeCategory === 'filmes' || activeCategory === 'admin') && selectedTheme && (
        <div className="w-full">
          ...
          <dialog id="digital-twin-modal" className="bg-transparent m-auto backdrop:bg-black/90 backdrop:backdrop-blur-sm fixed inset-0 z-[100] outline-none">
             <div className="flex flex-col items-center gap-2 bg-dark-900/80 p-6 md:p-12 rounded-[3rem] border border-dark-700 shadow-2xl relative w-[95vw] max-w-xl animate-scale-up overflow-hidden">
                <button onClick={() => { document.getElementById('digital-twin-modal').close(); setSelectedMovie(null); }} className="absolute top-6 right-6 w-9 h-9 bg-black/50 hover:bg-red-500/20 text-zinc-400 hover:text-red-400 border border-white/10 rounded-full flex items-center justify-center transition-all z-10">✕</button>
                ...
                <div className="w-full flex gap-2 mt-2">
                   <button onClick={() => document.getElementById('digital-twin-modal').close()} className="flex-1 py-1.5 font-black rounded-md bg-dark-700 hover:bg-dark-600 text-white uppercase text-sm border border-white/10 transition">
                      VOLTAR
                   </button>
                   <button onClick={handleExportBanner} className="flex-[2] py-1.5 font-black rounded-md bg-gradient-to-r from-brand-500 to-orange-500 hover:from-brand-400 hover:to-orange-400 text-white uppercase text-sm shadow-xl shadow-brand-500/30 transition transform hover:scale-[1.02]">
                      ⬇️ BAIXAR ARTE HD
                   </button>
                </div>
             </div>
      ...
      {!selectedTheme && mode !== 'auto' && (
        <div className="flex flex-col items-center justify-center py-8 text-zinc-600 border border-dashed border-dark-700 rounded-md">
        ...
        <div className="space-y-2 animate-slide-up">
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
              <div>
                <h2 className="text-sm font-black text-white">Oficina de <span className="text-brand-500">Modelos Elite</span></h2>
              ...
              <button 
                ...
                className="bg-brand-500 hover:bg-brand-600 text-white font-bold px-2 py-1 rounded-md transition-all shadow-xl shadow-brand-500/20 flex items-center gap-2 hover:scale-105 active:scale-95"
              >
              ...
              {adminTemplates.map(temp => (
                <div key={temp.id} className="group relative bg-dark-800 border border-white/5 rounded-[2.5rem] overflow-hidden hover:border-brand-500/50 transition-all shadow-2xl">
                  <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                    ...
                    <span className="absolute top-2 left-4 px-3 py-1 rounded-full bg-brand-500/20 border border-brand-500/30 text-[10px] font-black uppercase text-brand-400 z-30 backdrop-blur-md">
                  ...
                  <div className="p-5 flex justify-between items-center bg-dark-900 border-t border-white/5">
                    <div>
                      <h3 className="text-white font-bold text-base">{temp.name}</h3>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleEditTheme(temp)} className="w-9 h-9 bg-white/5 text-zinc-400 hover:text-brand-500 hover:bg-brand-500/10 rounded-full flex items-center justify-center transition-all border border-white/5"><Settings size={18} /></button>
                      <button onClick={() => handleDeleteTheme(temp.id)} className="w-9 h-9 bg-white/5 text-zinc-400 hover:text-red-500 hover:bg-red-500/10 rounded-full flex items-center justify-center transition-all border border-white/5"><Trash2 size={18} /></button>
                    </div>
                  </div>
                </div>
              ))}
           ...
           {adminTemplates.length === 0 && (
             <div className="py-10 text-center bg-dark-800/30 border border-dashed border-white/10 rounded-[3rem]">
                <Palette size={48} className="mx-auto text-zinc-700 mb-2" />
                <h3 className="text-sm font-bold text-white mb-2">Sua Oficina está vazia</h3>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-2">
          ...
          <div className="relative bg-dark-800 border border-white/10 rounded-[3rem] w-full max-w-7xl h-[90vh] overflow-hidden flex flex-col shadow-[0_0_100px_rgba(0,0,0,1)] animate-scale-in">
            ...
            <div className="p-6 border-b border-white/5 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-2">
                <div className="h-8 w-8 bg-brand-500/20 rounded-md flex items-center justify-center text-brand-500 border border-brand-500/30">
                  <Sparkles size={24} />
                </div>
                <div>
                  <h2 className="text-sm font-black text-white">{editingTheme ? 'Editar' : 'Novos'} <span className="text-brand-500">Protocolos Visuais</span></h2>
                  <p className="text-zinc-400 text-sm mt-1">Defina as proporções e camadas do modelo mestre.</p>
                </div>
              </div>
              <button 
                onClick={() => setShowThemeModal(false)} 
                className="w-9 h-9 flex items-center justify-center text-zinc-500 hover:text-white transition bg-white/5 rounded-md border border-white/5 hover:bg-red-500/20 hover:border-red-500/30"
              >
                <X size={24} />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col lg:flex-row p-6 gap-2">
              {/* FORMULÁRIO */}
              <form onSubmit={handleSaveTheme} className="flex-1 space-y-2">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-black/40 border border-white/10 rounded-md py-1.5 px-6 text-white placeholder-zinc-700 outline-none focus:border-brand-500 transition-all shadow-inner focus:ring-4 focus:ring-brand-500/10 font-bold" 
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <select 
                      ...
                      className="w-full bg-black/40 border border-white/10 rounded-md py-1.5 px-6 text-white outline-none focus:border-brand-500 transition-all appearance-none cursor-pointer font-bold"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                   <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                         <button type="button" onClick={() => setPreviewActive('feed')} className={`flex-1 py-1.5 rounded-md border text-[10px] font-black uppercase transition-all ${previewActive === 'feed' ? 'bg-brand-500 border-brand-500 text-white shadow-lg shadow-brand-500/20' : 'bg-black/20 border-white/5 text-zinc-600'}`}>FEED (1:1)</button>
                         <button type="button" onClick={() => setPreviewActive('story')} className={`flex-1 py-1.5 rounded-md border text-[10px] font-black uppercase transition-all ${previewActive === 'story' ? 'bg-brand-500 border-brand-500 text-white shadow-lg shadow-brand-500/20' : 'bg-black/20 border-white/5 text-zinc-600'}`}>STORY (9:16)</button>
                      </div>
                   </div>
                   <div className="space-y-2">
                      <label className="text-xs font-black text-zinc-500 uppercase tracking-[0.2em] block ml-1">Cromatismo Elite</label>
                      <div className="flex-1 bg-black/40 border border-white/10 rounded-md flex items-center p-1 px-4 gap-2 h-[58px] shadow-inner">
                         <input type="color" value={themeFormData.config.text_color} onChange={e => setThemeFormData({...themeFormData, config: {...themeFormData.config, text_color: e.target.value}})}
                            className="h-8 w-8 bg-transparent cursor-pointer rounded-md border-none" />
                   ...
                   <div className="space-y-2">
                      ...
                      <select 
                        ...
                        className="w-full bg-black/40 border border-white/10 rounded-md h-[58px] px-6 text-white outline-none focus:border-brand-500 transition-all appearance-none cursor-pointer font-extrabold text-sm shadow-inner"
                      >
                   ...
                   <div className="space-y-2">
                      ...
                      <button 
                        ...
                        className={`w-full h-[58px] rounded-md border text-[10px] font-black uppercase transition-all flex items-center justify-center gap-2 ${themeFormData.config.show_synopsis ? 'bg-brand-500/20 border-brand-500 text-brand-500' : 'bg-black/20 border-white/5 text-zinc-600'}`}
                      >
                ...
                <div className="space-y-2 bg-black/40 p-5 rounded-[2rem] border border-white/5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <div className="space-y-2">
                      ...
                      <input value={themeFormData.config.brand_name} onChange={e => setThemeFormData({...themeFormData, config: {...themeFormData.config, brand_name: e.target.value}})} 
                        placeholder="Ex: TV MAXX" className="w-full bg-black/40 border border-white/10 rounded-md py-1.5.5 px-6 text-sm text-white placeholder-zinc-700 outline-none focus:border-brand-500/50 transition-all font-bold" />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input value={themeFormData.config.brand_logo_url} onChange={e => setThemeFormData({...themeFormData, config: {...themeFormData.config, brand_logo_url: e.target.value}})} 
                        placeholder="Link da sua logo..." className="w-full bg-black/40 border border-white/10 rounded-md py-1.5.5 px-6 text-sm text-white placeholder-zinc-700 outline-none focus:border-brand-500/50 transition-all font-mono text-[10px]" />
                    </div>
                  </div>

                  <h4 className="text-[10px] font-black text-zinc-500 flex items-center gap-2 uppercase tracking-widest border-b border-white/5 pb-2 mt-2">
                  ...
                  <div className="space-y-2">
                    <div className="relative group">
                      ...
                      <input value={themeFormData.bg_url} onChange={e => setThemeFormData({...themeFormData, bg_url: e.target.value})} 
                        placeholder="Cole a URL da imagem de fundo..." className="w-full bg-black/40 border border-white/10 rounded-md py-1.5.5 pl-12 pr-6 text-sm text-white placeholder-zinc-700 outline-none focus:border-brand-500/50 transition-all font-mono font-bold" />
                    </div>
                    <div className="relative group">
                      ...
                      <input value={themeFormData.overlay_url} onChange={e => setThemeFormData({...themeFormData, overlay_url: e.target.value})} 
                        placeholder="Cole a URL do overlay transparente..." className="w-full bg-black/40 border border-white/10 rounded-md py-1.5.5 pl-12 pr-6 text-sm text-white placeholder-zinc-700 outline-none focus:border-brand-500/50 transition-all font-mono font-bold" />
                    </div>
                  </div>
                </div>

                <div className="pt-6">
                  <button type="submit" className="w-full h-14 bg-gradient-to-r from-brand-600 to-orange-500 text-white font-black rounded-[1.5rem] shadow-2xl shadow-brand-500/30 hover:scale-[1.02] active:scale-95 transition-all text-base flex items-center justify-center gap-2">
              ...
              {/* LIVE DUAL PREVIEW */}
              <div className="lg:w-[450px] space-y-2">
                 <div className="bg-black/60 border border-white/5 rounded-md p-2 flex flex-col items-center gap-2 shadow-2xl">
                    ...
                    <div className={`relative bg-zinc-950 border-4 border-white/5 rounded-3xl shadow-[0_30px_90px_rgba(0,0,0,1)] overflow-hidden transition-all duration-500 ${previewActive === 'feed' ? 'aspect-square w-full' : 'aspect-[9/16] h-[650px]'}`}>
                        ...
                        <div className="absolute top-2 right-4 z-[60]">
                           <button 
                             ...
                             className="px-3 py-1.5 bg-black/60 backdrop-blur-md border border-white/10 rounded-md text-[8px] font-black text-white hover:bg-brand-500/20 transition-all uppercase tracking-widest"
                           >
                    ...
                    <div className="bg-brand-500/5 border border-brand-500/20 p-2 rounded-md flex gap-2">
...
const FootballConfigurator = ({ theme, sportsData = [], loading }) => {
  ...
  return (
    <div className="bg-dark-800 border border-dark-700 rounded-md overflow-hidden shadow-xl">
      {/* Header */}
      <div className="px-2 py-1 border-b border-dark-700 flex items-center justify-between bg-dark-900/50">
      ...
      <div className="p-5 space-y-2">
        ...
        {(sportsData.length > 0 || loading) && (
          <div className="bg-brand-500/5 border border-brand-500/10 rounded-md p-2">
            <h4 className="text-[10px] font-black text-brand-400 uppercase tracking-widest mb-2 flex items-center gap-2">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
                {sportsData.flatMap(cat => cat.jogos || []).map((game, idx) => (
                  <button
                    ...
                    className="flex-shrink-0 bg-dark-900 border border-dark-700 hover:border-brand-500/50 p-2 rounded-md text-left transition group"
                  >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)}
              className="w-full bg-dark-900 border border-dark-600 rounded-md px-2 py-1.5 text-white text-sm focus:border-brand-500 outline-none transition"
        ...
        <div className="space-y-2">
          ...
          {matches.map((match, i) => (
            <div key={i} className="bg-dark-900 border border-dark-600 rounded-md p-2 space-y-2 relative">
              <button onClick={() => removeMatch(i)}
                className="absolute top-2 right-3 text-zinc-600 hover:text-red-400 transition text-sm">✕</button>
              <div>
                ...
                <input value={match.league} onChange={e => updateMatch(i, 'league', e.target.value)}
                  className="w-full bg-dark-800 border border-dark-700 rounded-md px-2 py-1 text-white text-sm focus:border-brand-500 outline-none transition mt-1"
                  placeholder="BRASILEIRÃO SÉRIE A" />
              </div>
              <div className="grid grid-cols-3 gap-2 items-center">
                <input value={match.team1} onChange={e => updateMatch(i, 'team1', e.target.value)}
                  className="bg-dark-800 border border-dark-700 rounded-md px-2 py-1 text-white text-sm font-bold text-center focus:border-brand-500 outline-none transition"
                  placeholder="TIME 1" />
                <div className="flex flex-col items-center gap-1">
                  <span className="text-brand-500 font-black text-base">VS</span>
                  <input value={match.time} onChange={e => updateMatch(i, 'time', e.target.value)}
                    className="w-full bg-dark-800 border border-dark-700 rounded-md px-2 py-1.5 text-yellow-400 text-xs font-bold text-center focus:border-yellow-500 outline-none transition"
                    placeholder="20:00" />
                </div>
        ...
        <button
          ...
          disabled={generating}
          className={`w-full font-bold py-1.5 rounded-md shadow-lg transition transform active:scale-95 flex items-center justify-center gap-2 text-sm
            ${done
...
const SportConfigurator = ({ category, theme, sportsData = [], loading }) => {
  ...
  return (
    <div className="bg-dark-800 border border-dark-700 rounded-md overflow-hidden shadow-xl">
      <div className="px-2 py-1 border-b border-dark-700 flex items-center gap-2 bg-dark-900/50">
      ...
      <div className="p-5 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2 items-center">
              <div>
                ...
                <input value={team1} onChange={e => setTeam1(e.target.value)}
                  className="w-full bg-dark-900 border border-dark-600 rounded-md px-2 py-1.5 text-white text-sm font-bold text-center focus:border-brand-500 outline-none transition"
              ...
              <div className="text-center">
                ...
                <input value={gameTime} onChange={e => setGameTime(e.target.value)}
                  className="w-full bg-dark-900 border border-dark-600 rounded-md px-2 py-1 text-yellow-400 text-sm font-bold text-center focus:border-yellow-500 outline-none transition"
                  placeholder="21:00" />
              </div>
              <div>
        ...
        {category === 'divulgacao' && (
          <div className="bg-dark-900 border border-dashed border-dark-600 rounded-md p-5 text-center text-zinc-500">
    ...
    <div className="bg-dark-800 border border-dark-700 rounded-md overflow-hidden shadow-xl">
      ...
      <div className="flex border-b border-dark-700 bg-dark-900/80">
        <button 
           ...
           className={`flex-1 py-1.5 text-sm font-black uppercase tracking-widest transition ${vodType === 'movie' ? 'text-brand-500 border-b-2 border-brand-500 bg-brand-500/5' : 'text-zinc-500 hover:text-white'}`}>
          🎬 Filmes
        </button>
        <button 
           onClick={() => { setVodType('series'); setVodCategory('all'); }} 
           className={`flex-1 py-1.5 text-sm font-black uppercase tracking-widest transition ${vodType === 'series' ? 'text-purple-500 border-b-2 border-purple-500 bg-purple-500/5' : 'text-zinc-500 hover:text-white'}`}>
          📺 Séries
        </button>
      </div>

      <div className="px-2 py-1 border-b border-dark-700 flex flex-col md:flex-row items-start md:items-center justify-between bg-dark-900/50 gap-2">
        <div className="relative w-full md:w-auto md:flex-1">
          ...
          <input
            ...
            className="w-full bg-dark-900 border border-dark-600 rounded-md pl-9 pr-4 py-1.5 text-white text-sm focus:border-purple-500 outline-none transition"
      ...
      {vodCategoriesList && vodCategoriesList.length > 0 && (
        <div className="bg-dark-900 px-2 py-1 border-b border-dark-700 overflow-x-auto scrollbar-none flex items-center gap-2">
          <button 
          ...
          <div className="space-y-2 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
            ...
            {search.length < 3 && (
              <>
                {trendingMovies.length > 0 && (
                  <div>
                    <h4 className="text-[10px] font-black text-brand-400 uppercase tracking-widest mb-2 flex items-center gap-2">🔥 Top 10 Filmes Hoje</h4>
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
                      {trendingMovies.map(c => (
                        <button key={c.id} onClick={() => handleSelect(c)} className={`flex-shrink-0 w-24 aspect-[2/3] rounded-md overflow-hidden border-2 transition-all ${selected?.id === c.id ? 'border-brand-500 scale-105' : 'border-dark-700 hover:border-zinc-500'}`}>
                ...
                {trendingSeries.length > 0 && (
                  <div>
                    <h4 className="text-[10px] font-black text-purple-400 uppercase tracking-widest mb-2 flex items-center gap-2">📺 Top 10 Séries Hoje</h4>
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                {filtered.slice(0, 30).map(c => (
                  <button
                    ...
                    className={`group relative rounded-md overflow-hidden aspect-[2/3] border-2 transition-all
                      ${selected?.id === c.id
            ...
            {filtered.length === 0 && search.length >= 3 && (
              <div className="text-center py-8 text-zinc-600">
                <span className="block text-base mb-2">🎬</span>
